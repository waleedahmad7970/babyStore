...
const BlogCard: React.FC<BlogCardProps> = ({
  ...
  return (
    <Link href={`/blog`} className="block text-inherit no-underline w-full">
      <div className="relative w-full overflow-hidden rounded-[12px] bg-primary  md:max-w-[450px] md:min-w-[450px]">
        <div className="relative h-[165px] w-full overflow-hidden rounded-[4px] md:h-[270px]">
          <Image
            src={
              image
                ? `https://www.babystore.ae/storage/back/assets/blogs/${image}`
            ...
            alt="tile"
            fill
            className="object-cover hover:transform hover:scale-125 duration-1000 "
          />
        </div>

        <div className="mt-[5px] md:mt-2 px-5 py-2">
          {/* <div className="max-w-max rounded-[4px] bg-[#FD71AF] p-[4.18px] md:p-[6.14px]">
            <p className="m-0 text-[10.74px] leading-[8px] font-medium text-white">
              {category}
            </p>
          ...
          <h3 className="font-inter min-h-[42.97px] text-[12px] leading-[140%] font-semibold text-[#000000] md:text-[20.34px] md:leading-[21.48px]">
            {title}
          </h3>
          <p
            style={{ color: "rgba(0, 0, 0, 0.50)" }}
            className="line-clamp-2 text-[9px] leading-[140%] font-normal md:text-[16.274px]"
          >
            {description}
          </p>
          <button className="rounded-full self-center my-3 bg-black px-6 py-2 font-medium text-white transition-all hover:bg-[#E7448A]">
            View Details
          </button>
        </div>
      </div>
